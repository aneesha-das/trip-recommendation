...
import pandas as pd
import numpy as np
data=pd.read_csv("temporary_working_documents/place_recommendation_raw_data.csv")
data_copy=data.copy(deep=True)
place_list=[];
data_copy.drop_duplicates(subset=['Name','Age Range', 'Place 1 (from India)'], keep='first', inplace=True)
data_copy=data_copy.reset_index()
data_copy.replace("Spllent", "Na", inplace=True)
...
data_copy.replace(["Na","Spllent"], np.nan, inplace=True)
data_copy.drop(columns=['index'],inplace=True)
data_copy.to_csv("temporary_working_documents/place_recommendation_place_filtered.csv", index=False)
